    ...
    def generate_signal(self, ind, data):
        ...
        if vr > self.vol_threshold:
            if price_up and rsi_val > 50:
                return SignalResult(signal="LONG", confidence=0.85, metadata={"type": "volume_confirmation_up"})
            elif not price_up and rsi_val < 50:
                return SignalResult(signal="SHORT", confidence=0.85, metadata={"type": "volume_confirmation_down"})
...
class OBVTrendFollow(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if obv_trend == price_trend:
            if obv_trend:
                return SignalResult(signal="LONG", confidence=0.8, metadata={"type": "confirmed_uptrend"})
            else:
                return SignalResult(signal="SHORT", confidence=0.8, metadata={"type": "confirmed_downtrend"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"obv_trend": obv_trend, "price_trend": price_trend})
...
class VWAPReversal(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if dev_val < -self.deviation_threshold and rsi_val < 35:
            return SignalResult(signal="LONG", confidence=0.85, metadata={"type": "vwap_oversold"})
        elif dev_val > self.deviation_threshold and rsi_val > 65:
            return SignalResult(signal="SHORT", confidence=0.85, metadata={"type": "vwap_overbought"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"deviation": dev_val, "rsi": rsi_val})
...
class ADAccumulation(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if ad_trend and macd_bullish:
            return SignalResult(signal="LONG", confidence=0.8, metadata={"type": "accumulation"})
        elif not ad_trend and not macd_bullish:
            return SignalResult(signal="SHORT", confidence=0.8, metadata={"type": "distribution"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={})
...
class ChaikinVolumeOscillator(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if osc_val > sig_val and rsi_val > 50:
            return SignalResult(signal="LONG", confidence=0.8, metadata={"type": "bullish_flow"})
        elif osc_val < sig_val and rsi_val < 50:
            return SignalResult(signal="SHORT", confidence=0.8, metadata={"type": "bearish_flow"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"osc": osc_val})
    ...
    def generate_signal(self, ind, data):
        ...
        if adx_val > 25:
            if plus_di.iloc[-1] > minus_di.iloc[-1]:
                return SignalResult(signal="LONG", confidence=0.85, metadata={"type": "strong_trend", "atr": float(atr_val)})
            else:
                return SignalResult(signal="SHORT", confidence=0.85, metadata={"type": "strong_trend", "atr": float(atr_val)})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"adx": float(adx_val)})
...
class BollingerWidthExpansion(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if width > self.width_threshold and vr > 1.3:
            if price_up:
                return SignalResult(signal="LONG", confidence=0.8, metadata={"type": "breakout_up"})
            else:
                return SignalResult(signal="SHORT", confidence=0.8, metadata={"type": "breakout_down"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"width": width, "vol": vr})
...
class KeltnerBollingerSqueeze(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if ratio < self.squeeze_threshold:
            if close > k_middle:
                return SignalResult(signal="LONG", confidence=0.75, metadata={"type": "squeeze_long", "ratio": ratio})
            else:
                return SignalResult(signal="SHORT", confidence=0.75, metadata={"type": "squeeze_short", "ratio": ratio})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"ratio": ratio})
...
class HistoricalVolatilityRange(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if percentile > self.hv_percentile_high and rsi_val > 65:
            return SignalResult(signal="SHORT", confidence=0.8, metadata={"type": "high_vol_sell"})
        elif percentile < self.hv_percentile_low and rsi_val < 35:
            return SignalResult(signal="LONG", confidence=0.8, metadata={"type": "low_vol_buy"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"hv_percentile": percentile})
    ...
    def generate_signal(self, ind, data):
        ...
        if adx_val > 25:
            if plus_di.iloc[-1] > minus_di.iloc[-1] and vr > 0.8:
                return SignalResult(signal="LONG", confidence=0.85, metadata={"type": "trending_expansion"})
            elif minus_di.iloc[-1] > plus_di.iloc[-1] and vr > 0.8:
                return SignalResult(signal="SHORT", confidence=0.85, metadata={"type": "trending_expansion"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"vol_ratio": vr, "adx": adx_val})
...
class MomentumROC(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if mom_val > self.threshold and roc_val > self.threshold:
            return SignalResult(signal="LONG", confidence=0.85, metadata={"type": "strong_momentum_up"})
        elif mom_val < -self.threshold and roc_val < -self.threshold:
            return SignalResult(signal="SHORT", confidence=0.85, metadata={"type": "strong_momentum_down"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"momentum": float(mom_val), "roc": float(roc_val)})
    ...
    def generate_signal(self, ind, data):
        ...
        if adx_val > self.adx_threshold:
            if plus_di.iloc[-1] > minus_di.iloc[-1] and mom_val > 0:
                return SignalResult(signal="LONG", confidence=0.9, metadata={"type": "strong_up_momentum"})
            elif minus_di.iloc[-1] > plus_di.iloc[-1] and mom_val < 0:
                return SignalResult(signal="SHORT", confidence=0.9, metadata={"type": "strong_down_momentum"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"momentum": float(mom_val), "adx": adx_val})
...
class MomentumVolumeRally(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if mom_val > self.mom_threshold and vr > self.vol_threshold:
            return SignalResult(signal="LONG", confidence=0.85, metadata={"type": "volume_rally"})
        elif mom_val < -self.mom_threshold and vr > self.vol_threshold:
            return SignalResult(signal="SHORT", confidence=0.85, metadata={"type": "volume_selloff"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"momentum": float(mom_val), "vol_ratio": vr})
...
class StochasticMomentumCombo(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        # Stochastic 金叉/死叉 + 動量確認
        k_above_d = k_val > d_val
        k_below_d = k_val < d_val
        
        if k_above_d and mom_val > 0 and k_val < 80:
            return SignalResult(signal="LONG", confidence=0.85, metadata={"type": "stoch_bullish"})
        elif k_below_d and mom_val < 0 and k_val > 20:
            return SignalResult(signal="SHORT", confidence=0.85, metadata={"type": "stoch_bearish"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"k": k_val, "d": d_val, "mom": float(mom_val)})
...
class WilliamsRMomentum(BaseStrategy):
    ...
    def generate_signal(self, ind, data):
        ...
        if w_val < self.williams_oversold and mom_val > 0:
            return SignalResult(signal="LONG", confidence=0.85, metadata={"type": "oversold_recovery"})
        elif w_val > self.williams_overbought and mom_val < 0:
            return SignalResult(signal="SHORT", confidence=0.85, metadata={"type": "overbought_decline"})
        return SignalResult(signal="HOLD", confidence=0.3, price=0.0, reason="Hold position", metadata={"williams": float(w_val), "momentum": float(mom_val)})
